...
checks={
'governance loaded':'js/v8-governance.js?v=8.1.1' in index,
...
'updated version':'8.1.1' in (root/'app-version.json').read_text(),
}
failed=[k for k,v in checks.items() if not v]
if failed:
 print('FALLÓ:', ', '.join(failed)); raise SystemExit(1)
print(f'Auditoría saneamiento V8.1.1: {len(checks)}/{len(checks)} controles OK')
