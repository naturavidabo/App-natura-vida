...
checks={
 'version 8.2.2':version.get('version')=='8.2.2',
 'script versioned':'js/v8-ai-assistant.js?v=8.2.2' in index,
 ...
 'structured output':'response_format' in fn and 'application/json' in fn and 'suggested_actions' in fn,
 ...
 'cache version':"nv-app-shell-v822" in sw,
 'under or equal 100 files':len(files)<=100,
}
failed=[k for k,v in checks.items() if not v]
if failed:
 print('FALLOS:',failed,'ARCHIVOS:',len(files)); sys.exit(1)
print(f'Motor IA V8.2.2: {len(checks)}/{len(checks)} controles OK · {len(files)} archivos')
