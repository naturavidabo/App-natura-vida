...
checks={
 'version 8.2.6':version.get('version')=='8.2.6',
 'scripts versioned':'js/v8-ai-assistant.js?v=8.2.6' in index and 'js/v8-seller-settlement.js?v=8.2.6' in index,
 'hybrid engine':'answerWithEngine' in ai and 'businessSnapshot' in ai and 'local-fallback' in ai,
 'operational drafts':all(x in ai for x in ['create_payment_plan','generate_receipt','seller_settlement','resolveDraftActionV825']),
 ...
 'smart floating assistant':'positionFabSmartV825' in ai and "fab.classList.toggle('left'" in ai and '.nvAiFab.left' in css,
 ...
 'seller settlement':'cashEventsForSeller' in settlement and 'Efectivo por entregar' in settlement and 'Cobros digitales' in settlement,
 'seller migration':'nv_seller_settlements' in settlement_sql and 'nv_payment_verifications' in settlement_sql,
 ...
 'structured actions':'draft_action' in fn and 'missing_fields' in fn and 'create_payment_plan' in fn,
 ...
 'cache version':"nv-app-shell-v826" in sw and 'v8-seller-settlement.js' in sw,
 ...
 'editable secretary actions':all(x in ai for x in ['Rechazar','Editar','Aprobar y continuar','applyEditedActionV826']),
...
print(f'Motor IA y caja V8.2.6: {len(checks)}/{len(checks)} controles OK · {len(files)} archivos')
