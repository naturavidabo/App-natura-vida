...
require('tests/audit_site_v802.py' in text,'workflow no ejecuta auditoría V8.0.2')
require('tests/audit_deployment.py' in text,'workflow no ejecuta auditoría de despliegue')
require('node tests/test_client_autocomplete_v802.js' in text,'workflow no prueba autocompletado de clientes')
...
if errors:
 print(f'Auditoría de despliegue V8.0.2: {checks-len(errors)}/{checks} controles OK')
 for e in errors: print('ERROR:',e)
 sys.exit(1)
print(f'Auditoría de despliegue V8.0.2: {checks}/{checks} controles OK')
