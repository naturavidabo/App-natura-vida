// auth.js — V6.7 — Fuente única de verdad: Supabase Auth.
...
async function registerNewAccount({ fullName, email, phone, city, password, activationCode } = {}) {
  ...
  const res = await signUpEmailAccount(cleanEmail, password, cleanName, {
    phone: cleanPhone, city: cleanCity, activationCode
