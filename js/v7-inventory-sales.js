...
(() => {
  ...
  function saleItems(){return Object.entries(saleCart).map(([id,qty])=>{
    ...
    const b=saleBreakdown(p); const q=Number(qty||0); const cost=linkedSellerV801()?0:resellerEffectiveCost(p); const signed=roundBs(b.unitPrice-b.basePrice);
    return {
