...
function renderResumen() {
  ...
  AppState.sales.forEach(v => {
    (v.items || []).forEach(it => {
      const p = AppState.products.find(x => x.id === it.productId);
      const cost = p ? grossCost(p) : 0;
      ganancia += it.subtotal - (cost * it.qty);
    });
  });
  ...
  if (filteredSales.length === 0) {
    ...
    html += filteredSales.slice(0, 60).map(v => {
      ...
      const summary = items.length === 1
        ? items[0].productName
        : `${items[0] ? items[0].productName : ''} +${items.length - 1} más`;
      ...
      <div class="histitem">
        <div class="l">
          ...
          <div class="meta">${escapeHtml(v.clientName || '—')} · ${v.type === 'unit' ? 'Unitaria' : 'Por mayor'} · ${totalUnits} unid. · ${fmtDate(v.date)}</div>
        </div>
