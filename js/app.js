        ...
        <div class="loginCard">
          ...
          ` : mode === 'register' ? `
            ...
            <form id="regForm" class="loginForm" autocomplete="on">
              ...
              <div class="field"><label>Codigo de activacion <span class="fieldOptional">(solo administradores)</span></label>
                <input type="password" id="rCode" autocomplete="off" inputmode="numeric" placeholder="Dejar vacio si eres representante"></div>
    ...
    if (mode === 'register') {
      $('#regForm').addEventListener('submit', async (e) => {
        ...
        const result = await registerNewAccount({
          ...
          activationCode: $('#rCode').value.trim()
        });
    ...
    $('#mainArea').innerHTML = `
      <section class="loginShell">
        <div class="loginCard">
          ...
          <p>Tu cuenta fue creada correctamente. El administrador debe aprobarla antes de que puedas operar.</p>
          <button class="btn block" id="pendingLogout">Cerrar sesion</button>
        </div>
      </section>
    `;
