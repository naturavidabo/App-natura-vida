          ...
          ${mode === 'login' ? `
            ...
            <form id="loginForm" class="loginForm" autocomplete="on">
              ...
              <div class="field"><label>Contrasena</label>
                <input type="password" id="lPass" autocomplete="current-password" placeholder="Contrasena" required></div>
              <button class="btn block" type="submit">Entrar</button>
            </form>
            <div class="loginLinks">
              <button class="linkBtn" id="goRegister">Crear cuenta nueva</button>
              <button class="linkBtn" id="goRecover">Olvide mi contrasena</button>
            </div>
          ` : mode === 'register' ? `
            <h2>Crear cuenta</h2>
            <form id="regForm" class="loginForm" autocomplete="on">
              ...
              <div class="field"><label>Contrasena</label>
                <input type="password" id="rPass" autocomplete="new-password" placeholder="Minimo 6 caracteres" required minlength="6"></div>
            ...
            <div class="loginLinks">
              <button class="linkBtn" id="goLogin">Ya tengo cuenta</button>
            </div>
          ` : `
            <h2>Recuperar acceso</h2>
            <p class="loginHint">Te enviaremos un enlace a tu Gmail para restablecer tu contrasena.</p>
            ...
            <div class="loginLinks">
              <button class="linkBtn" id="goLogin">Volver a iniciar sesion</button>
            </div>
          `}
        </div>
      </section>
...
async function afterLoginSuccess(result) {
    ...
    $('#mainArea').innerHTML = `
      <section class="loginShell">
        <div class="loginCard">
          ...
          <p>Tu cuenta fue creada correctamente. El administrador debe aprobarla antes de que puedas vender o sincronizar datos.</p>
          <button class="btn block" id="checkApproval">Revisar aprobación</button>
          <button class="btn outline block" id="pendingLogout">Cerrar sesión</button>
        </div>
      </section>
    `;
    $('#checkApproval').addEventListener('click', async () => {
      const btn = $('#checkApproval');
      btn.disabled = true;
      ...
      if (online && online.user && online.profile) {
        ...
        if (!AppState.session.pendingApproval) {
          await afterLoginSuccess({ ok: true, user: online.user });
          return;
        }
      }
      btn.disabled = false;
      btn.textContent = 'Revisar aprobación';
      showToast('La cuenta todavía está pendiente.', 'error');
    });
    $('#pendingLogout').addEventListener('click', () => logoutSession());
    return;
  }
  showToast('Sesión iniciada. Sincronizando con Supabase…');
  renderBottomNav();
  ...
  if (window.syncAfterLogin) {
    ...
    if (syncResult && syncResult.ok === false) {
      showToast(syncResult.message || 'No se pudo completar la sincronización.', 'error');
    }
  }
  await loadAllState();
  render();
...
function renderInicio() {
  ...
  main.innerHTML = `
    <section class="dashHero">
      ...
      <p>Sesión activa: <strong>${escapeHtml(AppState.session.fullName || '')}</strong> · ${escapeHtml(AppState.session.roleName || '')}. Supabase es la base oficial; este dispositivo sólo mantiene caché temporal y cola offline.</p>
    ...
    <section class="dashboardPanel">
      ...
      <div class="systemBadges">
        <span>Supabase oficial</span><span>PWA</span><span>Caché temporal</span><span>APK futuro</span><span>${pendingQuotes} cotización(es)</span>
      </div>
    </section>
  ...
  $('#mainArea').innerHTML = `
    <section class="dashboardPanel">
      <div class="panelHeader">
        ...
        <button class="btn sm outline" id="refreshUsersBtn">Actualizar</button>
      </div>
      <div class="miniStats">
...
async function initApp() {
  // V6.7: ya no se cargan datos de semilla (ensureBootstrapData) porque
  // IndexedDB ya no es fuente de usuarios ni de configuracion inicial.
  // Solo se abre la DB para la cola offline y la cache de lectura temporal.
  await openDB();
  await loadAllState();
  renderTopHeader();
