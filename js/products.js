      ...
      <div class="card" data-id="${p.id}">
        <div class="cardtop">
          ...
          <div class="info">
            ...
            <div class="priceline">
              <span class="p1">abastec.: ${fmtMoney(p.supplyPrice)}</span>
            </div>
            <div class="costline">costo neto: ${fmtMoney(cost)}</div>
          </div>
        </div>
...
function openProductForm(id) {
  ...
  const html = `
    ...
    <div class="sectiontitle2"><span>Precio de abastecimiento</span></div>
    <div class="field money">
      <label>Precio al que se entrega a la red de venta</label>
      <input type="number" inputmode="decimal" step="0.01" id="f_supplyprice" placeholder="0.00" value="${p ? p.supplyPrice : ''}">
    </div>
  ...
  openSheet(html, (overlay, close) => {
    ...
    $('#saveForm', overlay).addEventListener('click', async () => {
      ...
      const data = {
        ...
        supplyPrice: parseFloat($('#f_supplyprice', overlay).value) || 0,
