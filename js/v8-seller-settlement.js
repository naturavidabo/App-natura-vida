/* NATURA VIDA V8.2.6 — Rendición de caja de vendedores y control de cobros digitales.
...
(function(){
  ...
  let cache={settlements:[],verifications:[],loading:false,error:''};
  ...
  function cashEventsForSeller(sellerId){
    const events=[];
    (window.AppState?.sales||[]).filter(s=>!s.deletedAt&&!s.deleted_at&&String(s.sellerId||s.seller_id||'')===String(sellerId)).forEach(s=>{
    ...
    (window.AppState?.receivablePayments||[]).filter(p=>!p.deletedAt&&p.status!=='voided'&&String(p.responsibleUserId||p.sellerId||p.createdBy||'')===String(sellerId)).forEach(p=>{
  ...
  async function loadRemote(sellerId){
    cache={settlements:[],verifications:[],loading:true,error:''};
    if(!navigator.onLine||!sb())return cache={...cache,loading:false,error:'Se necesita conexión para consultar rendiciones.'};
    try{
      const client=sb();
      const [a,b]=await Promise.all([
        client.from(TABLE_SETTLEMENTS).select('*').eq('seller_user_id',sellerId).order('created_at',{ascending:false}).limit(100),
        client.from(TABLE_VERIFY).select('*').eq('seller_user_id',sellerId).order('created_at',{ascending:false}).limit(300)
      ]);
      const error=a.error||b.error;if(error)throw error;
      cache={settlements:a.data||[],verifications:b.data||[],loading:false,error:''};
    }catch(error){cache={settlements:[],verifications:[],loading:false,error:String(error?.message||error||'No se pudo cargar.')};}
    return cache;
  }
  ...
  async function renderSellerSettlementV825(){
    ...
    main.innerHTML=`<section class="v7PageHead nv825CashHead"><span class="v7Eyebrow">Control de dinero recibido</span><h1>${roleNow()==='field_seller'?'Mi rendición de caja':'Rendiciones de vendedores'}</h1><p>Separa efectivo bajo custodia del vendedor y cobros digitales que ingresan directamente a la cuenta autorizada.</p></section>
      <section class="nv825SettlementToolbar">${sellerSelector()}<button class="btn" id="nv825NewSettlement" ${t.cashAvailable<=0?'disabled':''}>Rendir efectivo</button></section>
      ${cache.error?`<div class="nv825MigrationAlert"><strong>Configuración pendiente</strong><span>${esc(cache.error)}</span><small>Ejecuta la migración V8.2.5 en Supabase para habilitar rendiciones compartidas y verificaciones.</small></div>`:''}
    ...
    document.getElementById('nv825NewSettlement')?.addEventListener('click',()=>openSettlementForm(t));
  ...
  Object.assign(window,{renderSellerSettlementV825,openSellerSettlementV825:()=>window.navigateTo?.('rendicion-caja'),NVSettlementV825:{VERSION,cashEventsForSeller,totalsForSeller}});
})();
