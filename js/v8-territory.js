/* NATURA VIDA V8.0.3 — registro territorial vinculado, panel de capas corregido y formularios móviles estables. */
(() => {
  const territory = {
  ...
  function mapPanelHtml(){
    return `<section class="v800MapPanel v802MapPanel" id="territoryMapPanelV801">
      ...
      <div class="v802MapLayerMenu ${territory.layersOpen?'':'hidden'}" id="mapLayerMenuV802">
        ...
        <div class="v803LayerFooter"><button type="button" id="closeLayersV803">Continuar</button></div>
      </div>
  ...
  function bindTerritoryControlsV801(){
    $all('.v800ViewTabs button').forEach(btn=>btn.onclick=()=>{closeMapLayersV803();closeMapSearchV802();territory.view=btn.dataset.view;patchTerritoryV801({rebuildContent:true});});
  ...
  function bindMapControlsV801(){
    ...
    $('#closeLayersV803')?.addEventListener('click',()=>{closeMapLayersV803();showToast('Vista del mapa aplicada.');});
    ...
    if(!document.body.dataset.nv803MapLayersBound){document.body.dataset.nv803MapLayersBound='1';document.addEventListener('click',event=>{if(territory.layersOpen&&!event.target.closest('#mapLayerMenuV802')&&!event.target.closest('#mapLayersV802'))closeMapLayersV803();});}
  }
  ...
  function openProspectFormV801(id='',prefill={}){
    ...
    openSheet(`<h2>${id?'Editar prospecto':'Registro territorial'} <span class="x" id="closeSheet">✕</span></h2><div class="v803TerritoryFormNotice"><strong>Busca antes de registrar</strong><span>Escribe un nombre y selecciona un cliente existente para completar WhatsApp, ciudad, dirección y georreferencia.</span></div>${canCreateForTeam()?`<div class="field"><label>Responsable de la cuenta</label><select id="tpOwner">${ownerOptions(current.ownerUserId||linkedClient?.ownerUserId||currentUid(),false)}</select></div>`:''}<div class="field"><label>Tienda, prospecto o cliente existente</label><div class="clientAutocompleteV802"><input id="tpBusiness" autocomplete="off" value="${esc(initialBusiness)}" placeholder="Ej.: Gloria, Biomujer o nombre de tienda"><div id="tpClientSuggestionsV803" class="clientSuggestionsV802 hidden"></div></div><small class="clientAssistV802">Desde 2 letras se muestran clientes parecidos. Al elegir uno se rellenan sus datos.</small></div><div id="tpLinkedClientV803" class="v803LinkedClient ${linkedClient?'':'hidden'}"></div><div class="field-row"><div class="field"><label>Persona de contacto</label><input id="tpName" value="${esc(initialName)}"></div><div class="field"><label>WhatsApp</label><input id="tpPhone" inputmode="tel" value="${esc(initialPhone)}"></div></div><div class="field-row"><div class="field"><label>Ciudad</label><input id="tpCity" value="${esc(initialCity)}"></div><div class="field" id="tpPotentialFieldV803"><label>Potencial</label><select id="tpPotential"><option value="low" ${current.potential==='low'?'selected':''}>Bajo</option><option value="medium" ${!current.potential||current.potential==='medium'?'selected':''}>Medio</option><option value="high" ${current.potential==='high'?'selected':''}>Alto</option></select></div></div><div class="field"><label>Dirección / referencia</label><input id="tpAddress" value="${esc(initialAddress)}"></div><div class="field" id="tpStatusFieldV803"><label>Estado</label><select id="tpStatus">${['prospect','contacted','qualified','converted','inactive','lost'].map(s=>`<option value="${s}" ${current.status===s?'selected':''}>${statusLabel(s)}</option>`).join('')}</select></div><div class="field-row"><div class="field"><label>Latitud</label><input id="tpLat" inputmode="decimal" value="${lat}"></div><div class="field"><label>Longitud</label><input id="tpLng" inputmode="decimal" value="${lng}"></div></div><button class="btn ghost block" id="tpGps">Capturar ubicación actual</button><div class="field"><label>Dato de ubicación</label><input id="tpLocation" value="${esc(initialLocation)}"></div><div class="field"><label>Observaciones</label><textarea id="tpNotes" rows="3">${esc(initialNotes)}</textarea></div><div class="stickyActions v803TerritoryActions"><button type="button" class="btn outline" id="cancelProspectV803">Cancelar</button><button class="btn" id="saveProspectV801">${linkedClient?'Actualizar cliente y ubicación':'Guardar prospecto'}</button></div>`,(overlay,close)=>{
      ...
      renderLinkedClient();
      if(window.bindClientAutocompleteV802)bindClientAutocompleteV802({input:businessInput,container:$('#tpClientSuggestionsV803',overlay),limit:7,onTyping:value=>{if(linkedClient){const selectedName=linkedClient.businessName||linkedClient.name||'';if(normalizeSearch(value)!==normalizeSearch(selectedName)){linkedClient=null;renderLinkedClient();}}},onSelect:fillFromClient});
      $('#closeSheet',overlay).onclick=close;$('#cancelProspectV803',overlay).onclick=close;
