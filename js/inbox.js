/* inbox.js — Buzón simple de pedidos, mensajes y novedades online/offline. */
...
async function openInboxPanel(forceRefresh = false) {
  ...
  openSheet(`
    ...
    <button class="btn outline block" id="refreshInboxBtn" style="margin-bottom:10px;">Actualizar buzón</button>
  ...
  `, (overlay, close) => {
    ...
    $('#refreshInboxBtn', overlay).addEventListener('click', async () => { close(); await openInboxPanel(true); });
