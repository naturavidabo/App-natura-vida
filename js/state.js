...
async function saveSettings() {
  ...
  if (window.requireAuth && requireAuth() && window.isOnlineConfigured && isOnlineConfigured()) {
    await queueSync('settings', row.key, 'put', row).catch(() => {});
    if (window.cloudAfterPut) cloudAfterPut('settings', row).catch(() => {});
  }
}
