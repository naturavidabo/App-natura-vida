...
/* Cascada de precios:
   1) publicPrice — precio fijo al público (venta unitaria)
   2) wholesaleBasePrice — público menos % de descuento general mayorista
   3) priceForGroup — el grupo ajusta ADEMÁS sobre el precio mayorista base (no sobre el público) */

function wholesaleBasePrice(product) {
  const pub = Number(product.publicPrice) || 0;
  const pct = Number(product.wholesaleDiscountPct) || 0;
  return Math.max(0, pub - (pub * pct / 100));
}

function priceForGroup(product, groupId) {
  const base = wholesaleBasePrice(product);
...
window.wholesaleBasePrice = wholesaleBasePrice;
