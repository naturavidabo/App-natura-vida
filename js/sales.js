...
function renderVender() {
  ...
  let html = `
    <div class="saletoggle">
      ...
      <button data-type="wholesale" class="${_saleType === 'wholesale' ? 'active' : ''}">Venta por mayor</button>
    </div>
  ...
  openSheet(html, (overlay, close) => {
    ...
    $('#confirmSale', overlay).addEventListener('click', async () => {
      ...
      for (const it of items) {
        ...
        saleItems.push({ productId: it.product.id, productName: it.product.name, qty: it.qty, unitPrice: it.price, subtotal: it.price * it.qty });
      }
