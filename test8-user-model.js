...
async function activateAdmin(backend, username, password) {
  ...
  await dev.registerNewAccount({ fullName:'Admin Test', email: username+'@gmail.com', phone:username, city:'La Paz', password, activationCode:'27121961' });
  return dev;
}

async function activateReseller(backend, username, password) {
  const dev = createDevice(backend);
  const email = username.replace(/[^a-z0-9]/gi, '') + '.rep@gmail.com';
  const reg = await dev.registerNewAccount({
    fullName: 'Rep ' + username, email, phone: username, city: 'Bolivia', password
  });
  if (!reg.ok && !reg.needsEmailConfirmation) throw new Error('No se pudo registrar representante: ' + reg.message);
  // El representante queda pendiente; para estas pruebas necesitamos que opere
  // => lo aprobamos directamente en el backend simulado
  const uid = dev.AppState.session && dev.AppState.session.onlineUserId;
  if (uid) {
    const row = backend.tables.profiles.get(uid);
    if (row) { row.status = 'activo'; backend.tables.profiles.set(uid, row); }
    if (dev.AppState.session) { dev.AppState.session.statusCanonical = 'activo'; dev.AppState.session.pendingApproval = false; }
  }
  return dev;
}
...
async function main() {
  ...
  // Esperar que el syncAfterLogin de admin1 complete en background antes de login de admin2
  await new Promise(r => setTimeout(r, 200));
  const admin2 = createDevice(backend);
  const admin2login = await admin2.loginWithEmail('70011111@gmail.com', 'claveAdmin1');
  ok('1c. admin2 (mismo usuario, otro celular) inicia sesión online', admin2login.ok && admin2.AppState.session && admin2.AppState.session.online === true);
