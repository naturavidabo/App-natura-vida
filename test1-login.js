...
async function activateReseller(backend, username, password) {
  ...
  const res = await dev.registerNewAccount({ fullName:'Rep '+username, email, phone:username, city:'Bolivia', password });
  if (!res.ok) throw new Error('No se pudo activar el representante de prueba: ' + JSON.stringify(res));
  return dev;
}
...
async function main() {
  ...
  // En el mock de pruebas el primero es admin; forzamos representante para probar el flujo real
  if (dev1.AppState.session) { dev1.AppState.session.roleName = 'Revendedor'; dev1.AppState.session.roleCanonical = 'representante'; }
  ...
  const dev2login = await dev2.loginWithEmail('70022222.rep@gmail.com', 'claveRep123');
  // Mismo forzado de rol que dev1: en el mock de prueba el primer usuario queda como admin
  if (dev2.AppState.session) { dev2.AppState.session.roleName = 'Revendedor'; dev2.AppState.session.roleCanonical = 'representante'; }
  ok('6b. El segundo celular puede iniciar sesion', dev2login.ok === true, JSON.stringify(dev2login));
  ...
  ok('6c. El resultado final refleja ambas ventas: 50 - 7 - 12 = 31',
    finalStockMap && finalStockMap.get('prod_crema') === 31, `stock final=${finalStockMap && finalStockMap.get('prod_crema')}`);
